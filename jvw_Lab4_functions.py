...
class SmartVectorLayer:
    ...
    def zonal_stats_to_field(self, raster_path, statistic_type="MEAN", output_field="ZonalStat"):
        ...
        #Your code
        try: # go through the fields in the feature class
            field_names = [field.name for field in arcpy.ListFields(self.feature_class)]
            if output_field in field_names: # if the field exists in the list, print this
                print(f"Field {output_field} already exists. Will overwrite values.")
            else: # if the field doen't exist, add it to the feature class
                arcpy.AddField_management(self.feature_class, output_field, "FLOAT")
                print(f"Added field {output_field} to store zonal statistics.")
        except Exception as e:
        ...
        try:
            arcpy.sa.ZonalStatisticsAsTable(
                self.feature_class, # feature class defining the zones
                "OBJECTID", # field to use as the zone
                raster_path, # path to the raster to calculate stats from
                temp_table, # temporary table to store results
                "DATA", # ignores null values
                statistic_type # type of statistic to calculate (mean)
            )
        ...
        # Your code
        try: # use the update cursor to go through the feature class and modify attributes
            with arcpy.da.UpdateCursor(self.feature_class, 
                                       ["OBJECTID", # zone identifier
                                        output_field]) as cursor: # field where stats will be stored
                for row in cursor:
                    object_id = row[0] # get the object ID from "OBJECTID" field
                    if object_id in zonal_results: # if there are results for this object ID
                        row[1] = zonal_results[object_id] # update the output field with the zonal stats
                        cursor.updateRow(row)
            print(f"Updated {output_field} field with zonal statistics")
        except Exception as e:
